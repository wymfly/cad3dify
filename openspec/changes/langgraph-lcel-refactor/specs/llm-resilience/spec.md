...
#### Scenario: Retry applied to LCEL chain builders
- **WHEN** `build_vision_analysis_chain()` constructs the LCEL Runnable
- **THEN** the LLM step SHALL be wrapped with `.with_retry(stop_after_attempt=3, wait_exponential_jitter=True)`
- **AND** the retry wraps the LLM invocation only (not the prompt formatting or output parsing)

#### Scenario: Retry applied to code gen chain
- **WHEN** `build_code_gen_chain()` constructs the LCEL Runnable
- **THEN** the LLM step SHALL be wrapped with `.with_retry(stop_after_attempt=3, wait_exponential_jitter=True)`

#### Scenario: Retry applied to compare and fix chains
- **WHEN** `build_compare_chain()` or `build_fix_chain()` constructs the LCEL Runnable
- **THEN** the LLM step SHALL be wrapped with `.with_retry(stop_after_attempt=3, wait_exponential_jitter=True)`
...
- **WHEN** `analyze_vision_node` is processing a drawing image via VL model
