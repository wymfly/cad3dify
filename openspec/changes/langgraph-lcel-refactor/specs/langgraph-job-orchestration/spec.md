...
- **AND** `analyze_vision_node` SHALL directly invoke the LCEL vision analysis chain via `await chain.ainvoke()` instead of delegating to `pipeline.analyze_vision_spec()` via `asyncio.to_thread`

#### Scenario: Drawing generation uses LCEL chains directly
- **WHEN** `generate_step_drawing_node` runs after user confirmation
- **THEN** the node SHALL:
  1. Call `ModelingStrategist.select(spec)` for strategy selection (unchanged, sync rule engine)
  2. Invoke `build_code_gen_chain()` via `await chain.ainvoke()` for code generation
  3. Execute the generated code via `SafeExecutor` (unchanged)
  4. Invoke the refiner subgraph for Compare→Fix cycles
- **AND** the node SHALL NOT call `pipeline.generate_step_from_spec()` or use `asyncio.to_thread()`
