...
- **THEN** a `job.organic_spec_ready` SSE event is dispatched with payload containing `prompt_en`, `shape_category`, `suggested_bounding_box`, `final_bounding_box`, `engineering_cuts`, `quality_mode`, `provider`
...
- **WHEN** user calls POST /api/v1/jobs/{id}/confirm with `confirmed_spec` containing the original spec values
...
- **WHEN** user calls POST /api/v1/jobs/{id}/confirm with `confirmed_spec.bounding_box` set to a different value
...
- **WHEN** user calls confirm with `confirmed_spec.quality_mode="high"` or `confirmed_spec.provider="tripo3d"`
...
- **WHEN** user modifies the `prompt_en` field in confirmed_spec
