...
### D3: organic 图片上传 → 保留独立上传端点

organic 图片上传（参考图）的 MIME 白名单（png/jpeg/webp）与图纸上传不同，且不需要触发图执行。保留 `/api/v1/organic/upload` 作为纯文件上传端点（仅返回 file_id），或整合到 `/api/v1/jobs/upload` 增加 `purpose` 参数区分。
...
- **[后处理超时]** → MeshProvider 可能耗时 3-5 分钟，期间 SSE 流保持活跃。无框架级超时（Provider 内部自带轮询超时）。如果 Provider 卡死，SSE 流会挂起。→ 可后续加 graph-level timeout，本次不加。
