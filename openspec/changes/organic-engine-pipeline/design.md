...
**策略：** organic router 在 `backend/main.py` 中通过配置开关（`ORGANIC_ENABLED`）控制挂载。重型依赖（manifold3d、pymeshlab）在 handler/processor 内部懒加载，而非 module-level import。依赖不可用时返回 503 而非崩溃整个 API。

**理由：** 确保 organic 管道的依赖问题不影响现有机械管道的可用性。

### D8: EngineeringCut discriminated union 设计

**策略：** `EngineeringCut` 使用 Pydantic discriminated union（`Literal` discriminator on `type` field），每种类型（`FlatBottomCut`、`HoleCut`、`SlotCut`）有独立的必填字段和数值边界约束。`OrganicConstraints.cuts` 使用 `Field(default_factory=list)` 避免可变默认值。
