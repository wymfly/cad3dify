"""V1 打印配置管理 API — CRUD 打印技术配置。

GET    /api/v1/print-profiles
GET    /api/v1/print-profiles/{name}
POST   /api/v1/print-profiles
PUT    /api/v1/print-profiles/{name}
DELETE /api/v1/print-profiles/{name}
"""
...
_CUSTOM_PROFILES_DIR = Path(__file__).parent.parent.parent / "knowledge" / "print_profiles"
...
async def create_profile(body: dict[str, Any]) -> dict[str, Any]:
    ...
    if p.name in _PRESET_NAMES:
        raise APIError(
            status_code=409,
            code=ErrorCode.PROFILE_PROTECTED,
            message=f"Cannot overwrite preset profile '{p.name}'",
        )
    d = _ensure_dir()
    ...
    if path.exists():
        raise APIError(
            ...
            message=f"Profile '{p.name}' already exists",
        )
    path.write_text(
        _safe_dump(p),
        encoding="utf-8",
...
async def update_profile(name: str, body: dict[str, Any]) -> dict[str, Any]:
    ...
    if name in _PRESET_NAMES:
        raise APIError(
            status_code=403,
            code=ErrorCode.PROFILE_PROTECTED,
            message=f"Cannot modify preset profile '{name}'",
        )
    d = _ensure_dir()
    path = d / f"{name}.yaml"
    if not path.exists():
...
async def delete_profile(name: str) -> dict[str, str]:
    ...
    if name in _PRESET_NAMES:
        raise APIError(
            status_code=403,
            code=ErrorCode.PROFILE_PROTECTED,
            message=f"Cannot delete preset profile '{name}'",
        )
    d = _ensure_dir()
    path = d / f"{name}.yaml"
    if not path.exists():
